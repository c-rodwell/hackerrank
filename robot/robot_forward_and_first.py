...
def best_total_score(config):
	...
	for step in range(1,N): #step = 1 through step = N - 1 : 2 cases, lose if energy becomes 0
		...
		if this_energy > 0:
			...
			newRoutes = [(energy-1, route_max_score+this_score) for (energy, route_max_score) in routes if (energy > 0 and (energy > this_energy or route_max_score+this_score > all_max_score))]
...
#intended: add state if better or non-comparable, but not equal or worse, to takeEnergy.
# def notDominatedBy(tuple1, tuple2):
# 	x1, y1 = tuple1
# 	x2, y2 = tuple2
