...
def best_total_score(config):
	...
	for step in range(1,N): #step = 1 through step = N - 1 : 2 cases, lose if energy becomes 0
		...
		if this_energy > 0:
			...
			newRoutes = [(energy-1, route_max_score+this_score) for (energy, route_max_score) in routes if (energy > 0 and notDominatedBy((energy-1, route_max_score+this_score), takeEnergy))]
