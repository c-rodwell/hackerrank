#do the robot problem iteratively with an explicit statck, since recursive version overflows stack

#parts:
#config - has the N, V, P for this problem
#state class - has methods for outcome states, computing its value
#stack - store the states we need to get to
#values - store the values for already computed states.
	...
	while stack:
		...
		if missing:
			for state in missing:
				stack.append(State(state)) #could check here if stack already has it, but I'm checking on remove instead.
		else:
			stack.pop()
