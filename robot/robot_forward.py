#instead of dynamic programming / recursive methods, do a forward approach taking advantage of problem structure.

#at step s, only s energy values are possible, and s increases by 1 each time.
#so search space is sum (s) for s from 1 to N  = N^2  only, unlike possibly exponential growth of a search tree.
#track total reward for each path going forward, then pick max at end.

#state is a tuple of step, energy, max reward.
#but can group them by step, then energy within step: 2d array or similar sturucture.
#energy index is different from energy value

#so store (energy, max score) at each spot.

# scores = [[None for e in range(s)] for s in range(1,N+1)]

# #scores[step][i] holds energy and max score for step s, ith distinct energy level
# startState = (0, 0)
# scores[0][0] = startState
